...
class CoverLetter:
	...
	def __init__(self, background_color=None, text_color=None):
		...
		self.text_color = text_color if text_color is not None else self.config.text_color
	...
	def set_page_config(self):
		...
		self.pdf_file.set_text_color(*self.text_color)
		self.pdf_file.rect(0, 0, self.pdf_file.w, self.pdf_file.h, 'F')

	def add_content(self, applicant_name, hr_name, email, phone, company, position, website, custom_body_text=None):
		...
		if custom_body_text:
			self.pdf_file.add_multi_cell(txt=custom_body_text)
		else:
